...
class UpdateDiscountServiceTest {
    ...
    void setUp() {
        ...
        CreateDiscountService createDiscountService = new CreateDiscountService(
                fakeType, fakeLoad, fakeSave, event -> {}, fakeChange, new FakeDiscountSnapshotPort());
        service = new UpdateDiscountService(fakeType, fakeLoad, fakeSave, createDiscountService,
                event -> {}, fakeChange, new FakeDiscountSnapshotPort());
    }

    private static Discount existing(boolean disabled) {
        return new Discount(DISCOUNT_ID, FLASH, List.of(new SkuTarget("SKU-1")), AmountType.PERCENT,
                new BigDecimal("10.00"), FROM, UPTO, disabled, "Original", "#FFFFFF", null, null, null, null);
    }

    private static UpdateDiscountUseCase.Command command() {
        return new UpdateDiscountUseCase.Command(
                DISCOUNT_ID, FLASH.id(), List.of(new SkuTarget("SKU-1")), AmountType.PERCENT,
                new BigDecimal("25.00"), FROM, UPTO, "Updated", "#00FF00", null, null, null, null);
    }

    @Test
    @DisplayName("Updating a discount writes one UPDATE ledger row with old/new snapshots and the acting user")
    void update_recordsLedgerRow_withOldAndNewSnapshots() {
        fakeLoad.store(existing(false));

        Discount saved = service.execute(command(), ACTOR_ID);

        assertEquals(1, fakeChange.saved.size());
        DiscountChange change = fakeChange.saved.get(0);
        assertEquals(saved.id(), change.discountId());
        assertEquals(ChangeType.UPDATE, change.changeType());
        assertNotNull(change.oldValueJson(), "old snapshot must reflect the pre-update state");
        assertNotNull(change.newValueJson());
        assertNotEquals(change.oldValueJson(), change.newValueJson(), "amount changed 10% -> 25%");
