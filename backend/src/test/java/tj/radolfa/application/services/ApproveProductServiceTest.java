...
class ApproveProductServiceTest {
    ...
    static ApproveProductService service(FakeStore store) {
        return new ApproveProductService(store, store);
    }
    ...
    @Test
    @DisplayName("PENDING_REVIEW → AWAITING_STOCK")
    void pendingReview_approvesToAwaitingStock() {
        FakeStore store = new FakeStore();
        store.put(withStatus(1L, ProductStatus.PENDING_REVIEW));

        service(store).execute(1L, 10L);

        assertEquals(ProductStatus.AWAITING_STOCK, store.get(1L).getStatus());
    }
