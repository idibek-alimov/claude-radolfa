...
| **Step 3:** Step 2 — Media Upload | ✅ COMPLETE | Created `imageUpload.ts` API (`POST /api/v1/admin/images/upload` with `image` field), built `Step2Media.tsx` with per-color tabs, drag-and-drop zone, sequential file uploads with per-image spinner placeholders, thumbnail grid with delete overlay, and error dismissal; wired into `ProductCreationWizard`. |
| **Step 4:** Step 3 — Variant Matrix | ✅ COMPLETE | Built `Step3VariantMatrix.tsx` with auto-seeded ONE_SIZE rows per color, flat table (color chip, size, barcode, price, stock), collapsible logistics columns, per-color "Apply to color" bulk-fill for price and stock, delete guard (min 1 row per color), inline red errors on empty sizeLabel/barcode, and `validateStep3`/`isStep3Valid` wired into orchestrator's `goNext()`. |
