...
| Phase 5 — i18n Keys | [x] | Added `common.loyaltyTierBadge`, `common.loyaltyTierDetail`, `productDetail.partialDiscountHint` to all 3 locales (en/ru/tj). `yourPrice` was already present. 0 compiler errors. |
| Phase 6 — Loyalty Dashboard Null-Tier Incentive | [x] | Replaced generic `earnPoints` paragraph in null-tier branch with a two-line incentive block: bold `startEarning` title + `spendToNextTier` sub-line (shown only when `loyalty.spendToNextTier != null`), falling back to `earnPoints` when no spend data. No new i18n keys — both strings already existed in all 3 locales. 0 compiler errors. |
| Phase 7 — frontend/CLAUDE.md Update | [x] | Replaced stale `### Price Model` section (which documented `minPrice/maxPrice/tierDiscountedMinPrice` and said "Never render loyaltyPrice") with the correct 8-field variant-level table, SKU-level fields note, 4-case display logic table, and backend formula note. Also corrected the `### Key Type Field Names` table — `Sku` and `ListingVariant` rows now list the old deleted fields and correct current fields. |
