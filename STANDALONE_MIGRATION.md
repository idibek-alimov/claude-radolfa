...
| 5 | [x] | Implemented `RegisterUserService` (implements `RegisterUserUseCase`): creates user with `USER` role, `enabled=true`, `LoyaltyProfile.empty()`. Wired into `OtpAuthService` — removed private `createNewUser()`, now delegates to the use case. Added `GET /api/v1/users/me` to `UserController` (was missing; SecurityConfig already protected the path). Compile clean, all tests pass. |
| 6 | [x] | Added `V3__cart_tables.sql` (carts + cart_items). Created `CartEntity`, `CartItemEntity`, `CartRepository`, `CartMapper`, `CartRepositoryAdapter` (implements `LoadCartPort` + `SaveCartPort`). Added `findVariantById(Long id)` to `LoadListingVariantPort` + implemented in `ProductHierarchyAdapter`. Implemented `AddToCartService`, `RemoveFromCartService`, `UpdateCartItemQuantityService`, `ClearCartService`, `GetCartService` (enriches each item with live stock + product display data). New `CartController` at `/api/v1/cart` with GET / POST items / PUT items/{skuId} / DELETE items/{skuId} / DELETE. Added cart rules to SecurityConfig. Compile clean, all tests pass. |
| 7 | [x] | No V4 migration needed — `external_order_id` was already nullable in V1 baseline. Added `DomainServiceConfig` to expose `LoyaltyCalculator` as a Spring bean. Implemented `CheckoutService` (cart→order: stock re-validation, tier discount via LoyaltyCalculator, points redemption, stock decrement, cart CHECKED_OUT), `UpdateOrderStatusService` (PENDING→PAID→SHIPPED→DELIVERED with transition guard), `CancelOrderService` (USER: own PENDING only; ADMIN: any non-final, restores stock). Added `POST /api/v1/orders/checkout`, `PATCH /api/v1/orders/{id}/cancel`, `PATCH /api/v1/orders/{id}/status`. SecurityConfig: ADMIN-only guard for status endpoint. Compile clean, all tests pass. |
| 8 | [x] | Added `V5__payments_table.sql` (payments table + completed_at + indexes). Created `PaymentEntity`, `PaymentRepository`, `PaymentMapper` (MapStruct), `PaymentRepositoryAdapter` (implements `SavePaymentPort` + `LoadPaymentPort`). Updated `Payment` domain record to carry `providerRedirectUrl`. Added `AwardLoyaltyPointsStub` (`@ConditionalOnMissingBean`) as Phase 9 placeholder. Implemented `InitiatePaymentService` (verify PENDING order + owner, call gateway, persist PROCESSING payment), `ConfirmPaymentService` (idempotency guard, complete payment, PAID order, trigger loyalty stub), `RefundPaymentService` (gateway refund, REFUNDED payment, cancel order + restore stock). New `PaymentController` (`POST /api/v1/payments/initiate/{orderId}`, `GET /api/v1/payments/{orderId}`, `POST /api/v1/payments/{orderId}/refund`), `PaymentWebhookController` (`POST /api/v1/webhooks/payment`). SecurityConfig: webhook permit-all, ADMIN-only refund guard. Compile clean, all tests pass. |
| 9 | [x] | Implemented `AwardLoyaltyPointsService` (replaces stub via `@ConditionalOnMissingBean`): loads user + order + tiers, calls `LoyaltyCalculator.awardPoints()`, persists updated profile. Implemented `RedeemLoyaltyPointsService`: validates available points, deducts pessimistically, returns monetary value. Refactored `CheckoutService`: removed inline points deduction block + dead `allTiers` load + `LoadLoyaltyTierPort`/`SaveUserPort` dependencies; now delegates to `RedeemLoyaltyPointsUseCase`. Added `loadRecentPaidByUserId` to `LoadOrderPort` + `OrderRepository` + `OrderRepositoryAdapter`. Created `GetRecentEarningsService` (approx cashback per recent PAID order using current tier). Extended `UserDto.LoyaltyDto` with `recentEarnings` list. Updated `UserController.getMe()` to include recent earnings. Compile clean, 14/14 tests pass. |
| 10 | [x] | Deleted entire `infrastructure/importer/` package (9 files), all 8 sync use case interfaces, all 7 sync service implementations, `ImportController`, 9 import DTOs, sync log infrastructure (entity/repo/adapter/port), idempotency infrastructure (entity/repo/adapter/port). Removed `spring-boot-starter-batch` from pom.xml. Renamed `ApiKeyAuthenticationFilter` → `ServiceApiKeyFilter` (now grants `ROLE_ADMIN` instead of `ROLE_SYNC`). Removed `UserRole.SYNC`. Updated SecurityConfig (removed `/api/v1/sync/**` route, SYNC references in `users/me`, `wishlist`, `orders` matchers), UserController, ListingController, SearchController, LoyaltyController, ChangeUserRoleService, ToggleUserStatusService, OpenApiConfig, ApiKeyProperties. Added `V6__drop_sync_infrastructure.sql` (drops `sync_log`, `import_idempotency`, all Spring Batch tables/sequences, migrates SYNC users to ADMIN, deletes SYNC role). Updated CLAUDE.md. Compile clean, 14/14 tests pass. Zero `UserRole.SYNC` or `ROLE_SYNC` references remain. |
